...
async function studentDashboard(){
 ...
 // Baca status tiap ujian langsung berdasarkan ID dokumen. Ini lebih andal daripada query
 // daftar attempt dan memastikan status selesai langsung mengubah tombol di dashboard.
 var attempts={};
 try{
  var attemptDocs=await Promise.all(state.exams.map(function(x){return db.collection('examAttempts').doc(attemptId(x.id)).get();}));
  attemptDocs.forEach(function(d,i){if(d.exists)attempts[String(state.exams[i].id)]=Object.assign({id:d.id},d.data());});
 }catch(e){console.warn('load direct attempts',e);}
...
async function examResultsAdmin(){
 ...
 top('Hasil Pengerjaan Ujian','<div class="wrap"><div class="card"><h2>Filter Hasil</h2><div class="grid"><select id="resultExam"><option value="">Semua ujian</option>'+exams.map(function(x){return '<option value="'+esc(x.id)+'">'+esc(x.name)+'</option>';}).join('')+'</select><select id="resultClass"><option value="">Semua kelas</option>'+classes.map(function(c){return '<option value="'+esc(c)+'">'+esc(c)+'</option>';}).join('')+'</select></div><div id="resultSummary"></div></div><div class="card"><div id="resultTable"></div></div></div>',admin,'Admin');
...
function renderExamResults(){
 ...
 var sm={};d.students.forEach(function(s){sm[s.id]=s;});var em={};d.exams.forEach(function(e){em[e.id]=e;});
 var rows=d.attempts.filter(function(a){return (!eid||String(a.examId)===eid)&&(!cls||String(a.classId)===cls);}).sort(function(a,b){return String(a.classId||'').localeCompare(String(b.classId||''),undefined,{numeric:true})||String((sm[a.studentId]||{}).name||a.nis||'').localeCompare(String((sm[b.studentId]||{}).name||b.nis||''));});
 var completed=rows.filter(function(a){return a.status==='completed';}).length,inprogress=rows.filter(function(a){return a.status==='in_progress';}).length;
 if(el('resultSummary'))el('resultSummary').innerHTML='<div class="result-stats"><span class="pill orange">Sudah Ujian: '+completed+'</span><span class="pill green">Sedang Mengerjakan: '+inprogress+'</span><span class="pill gray">Total Attempt: '+rows.length+'</span></div>';
 var html='<div class="table-wrap"><table class="table"><tr><th>Kelas</th><th>NIS</th><th>Nama</th><th>Ujian</th><th>Status</th><th>Aksi</th></tr>'+rows.map(function(a){var st=sm[a.studentId]||{},ex=em[a.examId]||{},done=a.status==='completed';return '<tr><td>'+esc(a.classId||st.classId||'-')+'</td><td>'+esc(a.nis||st.nis||'-')+'</td><td>'+esc(st.name||'-')+'</td><td>'+esc(ex.name||a.examId||'-')+'</td><td><span class="pill '+(done?'orange':'green')+'">'+(done?'Sudah Ujian':'Sedang Mengerjakan')+'</span></td><td>'+(done?'<button class="btn green small result-reset" data-id="'+esc(a.id)+'" data-name="'+esc(st.name||a.nis||'siswa')+'">Aktifkan Ulang</button>':'-')+'</td></tr>';}).join('')+'</table></div>';
 if(!rows.length)html='<div class="empty">Belum ada data pengerjaan untuk filter ini.</div>';if(el('resultTable'))el('resultTable').innerHTML=html;
