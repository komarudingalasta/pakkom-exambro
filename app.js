...
var classList=[],adminStudents=[],examTimerHandle=null;
...
async function studentDashboard(){
 ...
 var now=new Date(),todayCount=0,completedCount=0,pendingToday=0;
 ...
 var hero='<section class="student-hero"><div><span class="eyebrow">PORTAL SISWA</span><h1>Halo, '+esc(state.student.name)+'</h1><p>Kelas '+esc(state.classId)+' • NIS '+esc(state.student.nis)+'</p></div><div class="student-session"><span class="session-dot"></span>Sesi aktif</div></section>';
 ...
 top('PakKom Exambro','<div class="wrap student-dashboard">'+hero+stats+'<div class="card schedule-card"><div class="section-head"><div><h2>Jadwal Ujian</h2><p class="muted">Pilih ujian sesuai jadwal. Status pengerjaan akan tetap tersimpan meskipun halaman direfresh.</p></div><span class="pill gray">'+state.exams.length+' ujian</span></div>'+cards+'</div></div>',studentLogout,'Keluar');
...
function resumeExam(x){
 ...
 app.innerHTML='<div class="exam-page"><div class="exam-topbar"><div class="exam-title"><span class="logo">P</span><div><b>'+esc(x.name)+'</b><span>'+esc(x.subject||'Ujian online')+'</span></div></div><div class="exam-timer"><span>Sisa Waktu</span><b id="examCountdown">--:--:--</b></div><button class="btn finish-btn" id="finishExam">Sudah Selesai Mengerjakan</button></div><div class="exam-info-bar"><span><b>'+esc(state.student.name)+'</b> • Kelas '+esc(state.classId)+'</span><span>Jadwal '+esc(timeOnly(x.startAt))+' – '+esc(timeOnly(x.endAt))+'</span><button class="btn gray small" id="fullBtn">Fullscreen</button></div><div class="exam-warning">Pastikan jawaban sudah dikirim sebelum menekan <b>Sudah Selesai Mengerjakan</b>. Refresh halaman tidak akan mengeluarkan Anda dari ujian.</div><div class="exam-frame-wrap"><iframe class="exam-frame" src="'+esc(u)+'" allow="fullscreen" referrerpolicy="no-referrer-when-downgrade"></iframe></div></div>';
 el('fullBtn').onclick=function(){var target=document.documentElement;if(target.requestFullscreen)target.requestFullscreen().catch(function(){});};el('finishExam').onclick=finishCurrentExam;startExamTimer(x);
}
async function finishCurrentExam(){
 ...
 try{var ref=db.collection('examAttempts').doc(attemptId(x.id)),at=await getAttempt(x.id);if(at&&at.status==='completed'){stopExamTimer();saveActiveExam('');state.currentExam=null;return studentDashboard();}await ref.set({examId:x.id,studentId:state.student.id,nis:state.student.nis,classId:state.classId,status:'completed',createdByAuthUid:(at&&at.createdByAuthUid)||auth.currentUser.uid,completedAt:firebase.firestore.FieldValue.serverTimestamp(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});stopExamTimer();saveActiveExam('');state.currentExam=null;await pakkomAlert('Ujian sudah selesai dan telah dikunci.');studentDashboard();}catch(e){if(b){b.disabled=false;b.textContent='Sudah Selesai Mengerjakan';}pakkomAlert('Status selesai gagal disimpan: '+(e.code||e.message));}
}
function studentLogout(){stopExamTimer();clearSession();home();}
