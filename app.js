...
async function classesAdmin(){
 ...
 try{
  ...
  return pakkomAlert('Password kelas tidak dapat dimuat. Pastikan Firestore Rules V17.0 sudah dipublish. '+(e.code||e.message));
 }
...
async function studentsAdmin(){
 ...
 try{
  ...
  return pakkomAlert('Password siswa tidak dapat dimuat. Pastikan Firestore Rules V17.0 sudah dipublish. '+(e.code||e.message));
 }
...
async function saveSingleScore(sid){var c=window._scoreCtx,eid=el('scoreExam').value,exam=c.exams.find(function(x){return x.id===eid;}),s=c.students.find(function(x){return x.id===sid;}),inp=document.querySelector('.score-input[data-student="'+sid+'"]'),note=document.querySelector('.note-input[data-student="'+sid+'"]'),raw=inp.value.trim();if(raw==='')return pakkomAlert('Masukkan nilai.');var score=Number(raw);if(!isFinite(score)||score<0)return pakkomAlert('Nilai tidak valid.');var old=(c.scores||[]).find(function(x){return x.examId===eid&&x.studentId===sid;});await db.collection('examScores').doc(scoreDocId(eid,sid)).set({examId:eid,studentId:sid,nis:String(s.nis||''),studentName:String(s.name||''),classId:String(s.classId||''),examName:String(exam.name||''),subject:String(exam.subject||''),score:score,note:note.value.trim(),published:old?old.published===true:false,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});scoresAdmin();}
