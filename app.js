...
async function editClass(id){
 var d=await db.collection('classes').doc(id).get();if(!d.exists)return alert('Kelas tidak ditemukan.');var x=d.data()||{};
 var name=prompt('Nama/tampilan kelas:',x.name||id);if(name===null)return;name=name.trim()||id;
 var p=prompt('Password kelas baru. Kosongkan jika password tidak diubah:','');if(p===null)return;if(p&&p.length<6)return alert('Password minimal 6 karakter.');
 var data={name:name,updatedAt:firebase.firestore.FieldValue.serverTimestamp()};if(p){data.passwordHash=await sha256(p);data.password=firebase.firestore.FieldValue.delete();data.demoPassword=firebase.firestore.FieldValue.delete();}
 await db.collection('classes').doc(id).update(data);alert('Data kelas diperbarui.');classesAdmin();
}
...
async function ensureClassExists(id){
 ...
 await ref.set({name:id,passwordHash:await sha256('123456'),active:true,createdFrom:'student-import',createdAt:firebase.firestore.FieldValue.serverTimestamp()});return true;
}
